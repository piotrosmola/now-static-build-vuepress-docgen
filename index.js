...
const { runNpmInstall, runPackageJsonScript } = require('@now/build-utils/fs/run-user-scripts.js') // eslint-disable-line import/no-extraneous-dependencies
...
exports.build = async ({ files, entrypoint, workPath, config }) => {
  if (path.basename(entrypoint) !== 'config.js') {
    throw new Error('config.js must be specified as entrypoint')
  }
